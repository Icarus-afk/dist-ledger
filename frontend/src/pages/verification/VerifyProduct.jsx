...
const VerifyProduct = () => {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      const response = await api.get(`/api/verify/product/${productData.serialNumber}`);
      
      // Check for potential inconsistencies in location data
      // For example, if it's "sold" but still shows at distributor
      if (response.currentLocation?.type === 'distributor' && 
        ...
        try {
          ...
          if (retailerCheck.success && retailerCheck.inventory) {
            ...
            const foundInRetailerInventory = retailerCheck.inventory.some(item => 
              item.serialNumbers && item.serialNumbers.includes(productData.serialNumber)
            );
      ...
      setError('Failed to verify product. Please check the serial number and try again.');
      setVerificationResult(null);
    } finally {
      setLoading(false);
    }
  ...
  const renderVerificationStatus = () => {
    ...
    if (!verificationResult.success) {
      return (
        <div className="p-6 bg-red-50 rounded-lg border border-red-200 text-center">
          ...
          <h3 className="text-lg font-medium text-red-800 mt-4">Verification Failed</h3>
          <p className="text-red-700 mt-2">{verificationResult.message || "Product not verified"}</p>
        </div>
      );
    }

    // Extract data from the response structure
    const { productInfo, currentLocation, authenticity, serialNumber, productId } = verificationResult;
    ...
    const getEntityTypeColor = (type) => {
      switch (type) {
        ...
        case 'distributor': return 'bg-yellow-500';
        case 'retailer': return 'bg-green-500';
    ...
    return (
      <div className="rounded-lg border bg-white overflow-hidden">
        <div className="p-6 bg-green-50 text-center border-b border-green-100">
          ...
          <h3 className="text-lg font-medium text-green-800 mt-4">Product Verified</h3>
          <p className="text-green-700">This product is authentic and registered in our system</p>
        </div>
        
        {inconsistencyDetected && (
          <div className="p-3 bg-amber-50 text-amber-800 border-b border-amber-100 text-sm">
            <div className="flex items-start">
              <svg className="w-5 h-5 mr-2 text-amber-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <div>
                <p className="font-medium">Location data inconsistency detected</p>
                <p>We've detected that this product's location information may have been updated more recently than shown in the blockchain records. The information displayed has been corrected.</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(currentLocation.status)}`}>
          ...
          <div className="mt-6">
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center mb-2">
                <span className={`w-3 h-3 rounded-full mr-2 ${getEntityTypeColor(currentLocation.type)}`}></span>
                <span className="font-medium capitalize">{currentLocation.type || 'Unknown'}</span>
                {currentLocation.entityId && (
                  <span className="text-xs text-gray-500 ml-2">({currentLocation.entityId})</span>
                )}
              </div>
              ...
              <p className="text-xs mt-1 capitalize">
                Status: <span className="font-medium">{getStatusText(currentLocation.status)}</span>
              </p>

              {currentLocation.saleId && (
                <p className="text-xs mt-1">
                  Sale ID: <span className="font-mono text-gray-700">{currentLocation.saleId}</span>
                </p>
              )}

              {currentLocation.returnId && (
                <p className="text-xs mt-1">
                  Return ID: <span className="font-mono text-gray-700">{currentLocation.returnId}</span>
                </p>
              )}
            ...
            <div className="mt-6">
              ...
              <div className="bg-gray-50 p-4 rounded">
                <p className="mb-2 flex items-center">
                  <span className={authenticity.isAuthentic ? "text-green-500 mr-2" : "text-red-500 mr-2"}>
                    {authenticity.isAuthentic ? "✓" : "✗"}
                  </span>
                  <span className="font-medium">
                    {authenticity.isAuthentic ? "Product is authentic" : "Authentication failed"}
                  </span>
                </p>
                
                {authenticity.rootMatchesRegistration !== undefined && (
                  <p className="flex items-center text-sm mb-2">
                    <span className={authenticity.rootMatchesRegistration ? "text-green-500 mr-2" : "text-red-500 mr-2"}>
                      {authenticity.rootMatchesRegistration ? "✓" : "✗"}
                    </span>
                    <span>Blockchain record {authenticity.rootMatchesRegistration ? "matches" : "does not match"} product registration</span>
                  </p>
                )}
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">
          Enter the product serial number to verify if the product is authentic and registered in our system.
        </p>
      </div>
        ...
        <div>
          <div className="bg-white shadow rounded p-6">
            ...
            <div className="mt-6 border-t pt-4">
              <h3 className="text-sm font-medium text-gray-700 mb-2">How to find your serial number</h3>
            ...
            verificationResult ? (
              ...
              <div className="bg-white shadow rounded p-6 flex items-center justify-center h-full">
                <div className="text-center p-8">
                  ...
                  <h3 className="text-lg font-medium text-gray-800">Ready to Verify Product</h3>
                  <p className="text-gray-600 mt-2">
                    Enter the product's serial number to check its authenticity and view its supply chain journey.
                  </p>
                </div>
              </div>
            )
